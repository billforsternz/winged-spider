...
{        
    if( gblFramedDiagram >= 0 )
    {
        ...
        if( gblPlaying )
            document.images[2+ (gblFramedDiagram+1)*64 + 4].src  = nextMove==-1?'png/blank.png':'png/mpause.png';
        else
            document.images[2+ (gblFramedDiagram+1)*64 + 4].src  = nextMove==-1?'png/blank.png':'png/mplay.png';
    }
}
    ...
    if( !err )
    {
            ...
            0x61<=hexDstFile && hexDstFile <= 0x68 && 0x31<=hexDstRank && hexDstRank <= 0x38 )
        {
            ...
            if( i<8 || (16<=i && i<24) || (32<=i && i<40)|| (48<=i && i<56) )
                ...
                w = ((i&1)==1);
            document.images[2+ gblFramedDiagram*64 + i].src = (w?'png/w.png':'png/y.png');
            $(".rover").css( 'top',  strSrcTop  );
            $(".rover").css( 'left', strSrcLeft );
            document.images[2+ (gblFramedDiagram+1)*64 + 2].src  = 'png/' + srcPng + '.png';
...
function cbShowMoveAfterAnimation()
{
    ...
    document.images[2+ (gblFramedDiagram+1)*64 + 1].src  = (prevMove==-1 ? 'png/blank.png' : 'png/mback.png');
    document.images[2+ (gblFramedDiagram+1)*64 + 7].src  = (nextMove==-1 ? 'png/blank.png' : 'png/mforward.png');
    
    // Clear the roving image - fixes the long-standing promotion bug
    document.images[2+ (gblFramedDiagram+1)*64 + 2].src  = 'png/blank.png';
    $(".rover").css( 'top',  '0px'  );
    $(".rover").css( 'left', '0px' );
    setPlayPauseImage();
    if( gblPlaying )
...
function pos( compressedStr, diagIdx, darkColour, label )
{
    ...
    {                    
        ...
        if( c == 'r' )
            ...
            fn = 'png/' + background;
        document.images[2+ diagIdx*64 + i].src  = fn;
    }
}
